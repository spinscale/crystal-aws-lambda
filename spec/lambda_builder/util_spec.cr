...
describe LambdaBuilder::Util do
  ...
  it "Lambda HTTP request parses" do
    ENV["_HANDLER"] = "foo"
  ...
  it "LambdaHTTPRequest works with empty body and query string" do
  ...
  describe "LambdaHTTPResponse" do
    it "always returns status code" do
      json = JSON.parse LambdaBuilder::Util::LambdaHttpResponse.new(123).to_json
    ...
    it "can contain a body as text" do
      text = "my text"
      json = JSON.parse LambdaBuilder::Util::LambdaHttpResponse.new(200, text).to_json
      json["body"]?.should eq text
    end

    it "can contain a body as json" do
      input = JSON.parse "{ \"foo\" : \"bar\" }"
      json = JSON.parse LambdaBuilder::Util::LambdaHttpResponse.new(200, input).to_json
      json["body"]["foo"]?.should eq "bar"
    end

    it "can contain additional headers" do
      response = LambdaBuilder::Util::LambdaHttpResponse.new(200, "body")
